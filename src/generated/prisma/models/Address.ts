...
export interface AddressDelegate<ExtArgs extends runtime.Types.Extensions.InternalArgs = runtime.Types.Extensions.DefaultArgs, GlobalOmitOptions = {}> {
   ...
   * 
   * // Only select the `addressLabel`
   * const addressWithAddressLabelOnly = await prisma.address.findMany({ select: { addressLabel: true } })
   * 
   */
   ...
   * })
   * 
   * // Create many Addresses and only return the `addressLabel`
   * const addressWithAddressLabelOnly = await prisma.address.createManyAndReturn({
   ...
   * })
   * 
   * // Update zero or more Addresses and only return the `addressLabel`
   * const addressWithAddressLabelOnly = await prisma.address.updateManyAndReturn({
   *   select: { addressLabel: true },
   *   where: {
   *     // ... provide filter here
   *   },
