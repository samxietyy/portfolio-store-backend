...
export interface ProductVariantDelegate<ExtArgs extends runtime.Types.Extensions.InternalArgs = runtime.Types.Extensions.DefaultArgs, GlobalOmitOptions = {}> {
   ...
   * 
   * // Only select the `sku`
   * const productVariantWithSkuOnly = await prisma.productVariant.findMany({ select: { sku: true } })
   * 
   */
   ...
   * })
   * 
   * // Create many ProductVariants and only return the `sku`
   * const productVariantWithSkuOnly = await prisma.productVariant.createManyAndReturn({
   *   select: { sku: true },
   *   data: [
   *     // ... provide data here
   *   ]
   ...
   * })
   * 
   * // Update zero or more ProductVariants and only return the `sku`
   * const productVariantWithSkuOnly = await prisma.productVariant.updateManyAndReturn({
   *   select: { sku: true },
   *   where: {
   *     // ... provide filter here
   *   },
